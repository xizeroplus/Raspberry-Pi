...
Gobang.prototype.computerDropChess = function(){
	var myScore = [], //玩家比分
		...
		maxScore = 0; //最大比分
//mark!!!
//post this.existChesses to computer
//get x,y
/*	 $.post("http://127.0.0.1/post/",
    {
      'data' : this.existChesses
    },
    function (data){        
    // data 可以是 xmlDoc, jsonObj, html, text, 等等.  
    //this;
    // 这个Ajax请求的选项配置信息，请参考jQuery.get()说到的this  
    var pointx=data.result[0];
     var pointy=data.result[1];
       }, 	"json");*/
	var value;
	$.ajax({
        async: false,
		 type : "POST",
		 url : "http://59.78.8.51:8080/post/",
		 data :   {
      'data' : this.existChesses
    }
	}).done(function(data){
...
/* $.post("http://127.0.0.1/mydict", function(data, status){
         alert("name: " + data.name + " age:" + data.age);})*/
    var pointx=value.result[0];
     var pointy=value.result[1];

 	this.checkChess(pointx, pointy)
	if (!this.over) {
		this.player = true;
	}
	return;
...
Gobang.prototype.currWinChesses = function(x, y, player){
	...
	for (var i = 1; i <= this.winsCount; i++) {
        ...
        async: false,
		 type : "POST",
		 url : "http://59.78.8.51:8080/scoreboard/",
		 data :   {
		 	'id': this.id,
      'data' : this.existChesses
    }
